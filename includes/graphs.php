...
function nms_graph_tree_nodes($tree_id, $parent_id = 0, $depth = 0, &$seen = array()) {
	if ($depth > 30 || isset($seen[$parent_id])) return array();
	$seen[$parent_id] = true;
	$nodes = array();
	foreach (get_allowed_tree_level((int) $tree_id, (int) $parent_id) as $item) {
		...
		$node['graphs'] = $node['kind'] === 'host' ? nms_graphs_for_host($item['host_id']) : array();
		$node['children'] = $node['kind'] === 'branch' ? nms_graph_tree_nodes($tree_id, $item['id'], $depth + 1, $seen) : array();
		if ($node['kind'] === 'graph') {
			$total = 0;
...
function nms_graph_node_count($nodes) {
	$count = 0;
	foreach ($nodes as $node) $count += count($node['graphs'] ?? array()) + (!empty($node['graph']) ? 1 : 0) + nms_graph_node_count($node['children'] ?? array());
	return $count;
}
