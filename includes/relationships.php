...
function nms_relationship_endpoint($encoded) {
	...
	if (!(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM host WHERE id = ? AND deleted = '' AND disabled = ''", array($host_id))) {
	...
	if ($query_id > 0) {
		...
		if ($identity === '') throw new InvalidArgumentException('The interface no longer exists in Cacti cache. Re-index and select it again.');
	}
	return array('host_id' => $host_id, 'query_id' => $query_id, 'index' => $index, 'identity' => $identity);
}
...
function nms_relationship_interface_identity($host_id, $query_id, $index) {
	$rows = db_fetch_assoc_prepared("SELECT field_name, field_value FROM host_snmp_cache
		WHERE host_id = ? AND snmp_query_id = ? AND snmp_index = ? AND field_name IN ('ifName', 'ifDescr')
		ORDER BY CASE field_name WHEN 'ifName' THEN 0 ELSE 1 END", array((int) $host_id, (int) $query_id, (string) $index));
	foreach ($rows as $row) {
		$value = trim((string) $row['field_value']);
		if ($value !== '') return nms_classification_text($row['field_name'] . ':' . $value, 512);
	}
	return '';
}
...
function nms_relationship_save($source, $target, $type) {
	if (!in_array($type, array('network', 'power', 'containment'), true)) throw new InvalidArgumentException('Select a supported relationship type.');
	$source = nms_relationship_endpoint($source);
	$target = nms_relationship_endpoint($target);
	if ($source['host_id'] === $target['host_id']) throw new InvalidArgumentException('Select two different devices.');
	$fingerprint = hash('sha256', json_encode(array($source['host_id'], $source['query_id'], $source['index'],
		$target['host_id'], $target['query_id'], $target['index'], $type, 'manual')));
	nms_category_execute("INSERT INTO plugin_nms_relationships
		...
		updated_by = VALUES(updated_by), updated_at = NOW(), archived_at = NULL", array($fingerprint,
		$source['host_id'], $source['query_id'], $source['index'], $source['identity'],
		$target['host_id'], $target['query_id'], $target['index'], $target['identity'], $type, nms_current_user_id()));
}
...
function nms_relationship_archive($id) {
	$row = db_fetch_row_prepared('SELECT * FROM plugin_nms_relationships WHERE id = ?', array((int) $id));
	if (!$row || $row['provenance'] !== 'manual') throw new InvalidArgumentException('Select a manual connection.');
	nms_require_device_access($row['source_host_id']);
	nms_require_device_access($row['target_host_id']);
	nms_category_execute('UPDATE plugin_nms_relationships SET archived_at = NOW(), updated_at = NOW(), updated_by = ? WHERE id = ?',
		array(nms_current_user_id(), (int) $id));
}
...
function nms_relationships($site_id) {
	...
	$rows = db_fetch_assoc_prepared("SELECT r.*, s.description AS source_name, t.description AS target_name
		...
		ORDER BY r.id", array((int) $site_id, (int) $site_id));
	foreach ($rows as &$row) {
		$row['identity_state'] = 'recorded';
		foreach (array('source', 'target') as $end) {
			if ((int) $row[$end . '_query_id'] > 0 && nms_relationship_interface_identity($row[$end . '_host_id'],
				$row[$end . '_query_id'], $row[$end . '_index']) !== $row[$end . '_identity']) {
...
function nms_relationship_options() {
	...
	$devices = db_fetch_assoc("SELECT h.id, h.description FROM host AS h WHERE h.deleted = '' AND h.disabled = '' AND $visible ORDER BY h.description");
	$options = array();
	foreach ($devices as $device) $options[$device['id'] . ':0:'] = $device['description'] . ' (device)';
	...
	foreach ($interfaces as $interface) {
		$options[$interface['id'] . ':' . $interface['snmp_query_id'] . ':' . $interface['snmp_index']] =
			$interface['description'] . ' · ' . ($interface['if_name'] !== '' ? $interface['if_name'] : $interface['if_description']) .
			' (query ' . $interface['snmp_query_id'] . ', index ' . $interface['snmp_index'] . ')';
	}
	return $options;
}
