 ...
 */
function nms_storage_execute($sql, $params = array()) {
...
function nms_category_execute($sql, $params = array()) {
...
function nms_category_migrate() {
	if (db_fetch_cell_prepared('SELECT meta_value FROM plugin_nms_meta WHERE meta_key = ?', array('equipment_categories_v1')) === 'complete') return;
	$lock = 'nms_categories_' . substr(hash('sha256', (string) db_fetch_cell('SELECT DATABASE()')), 0, 32);
	if ((int) db_fetch_cell_prepared('SELECT GET_LOCK(?, 10)', array($lock)) !== 1) {
		throw new RuntimeException('NMS category upgrade is busy. Retry after the current upgrade completes.');
	}
	try {
		if (db_fetch_cell_prepared('SELECT meta_value FROM plugin_nms_meta WHERE meta_key = ?', array('equipment_categories_v1')) === 'complete') return;
		if ((int) db_fetch_cell("SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE()
			AND TABLE_NAME = 'plugin_nms_device_categories'") &&
			(int) db_fetch_cell('SELECT COUNT(*) FROM plugin_nms_device_categories')) {
			throw new RuntimeException('Legacy category storage still exists. Confirm whether references are legacy category IDs or tree IDs before migration; no records were translated.');
		}
		...
		if (!is_array($references)) throw new RuntimeException('Could not read legacy category references.');
		foreach ($references as $reference) {
			$name = $reference['name'] === null ? 'Legacy tree #' . (int) $reference['legacy_tree_id'] . ' (missing)' : $reference['name'];
			nms_category_execute('INSERT INTO plugin_nms_categories (name, description, updated_at)
				VALUES (?, ?, NOW())', array($name, 'Preserved from Cacti tree ID ' . (int) $reference['legacy_tree_id'] . '; independent equipment category.'));
			$id = (int) db_fetch_cell('SELECT LAST_INSERT_ID()');
			if ($id < 1) throw new RuntimeException('Could not allocate an equipment category.');
			nms_category_execute('INSERT INTO plugin_nms_category_migration
				(legacy_tree_id, category_id, legacy_name, migrated_at) VALUES (?, ?, ?, NOW())',
				array($reference['legacy_tree_id'], $id, $name));
		}
		...
		foreach (array('plugin_nms_category_templates', 'plugin_nms_fault_rules', 'plugin_nms_snmprec_imports') as $table) {
			nms_category_execute('UPDATE ' . $table . ' AS r INNER JOIN plugin_nms_category_migration AS m
		...
		$catalogue = array('network' => 'Network', 'voice-video' => 'Voice/Video', 'security' => 'Security',
			'vsat' => 'Satellite/VSAT', 'los' => 'LOS Communications', 'computers' => 'Computers',
			'power' => 'Power', 'timing' => 'Timing', 'sensors' => 'Sensors/Instrumentation', 'fire' => 'Fire Prevention');
		$order = 0;
		foreach ($catalogue as $key => $name) {
			// Do not merge similarly named legacy categories: that would merge rule scopes.
			if (!(int) db_fetch_cell_prepared('SELECT COUNT(*) FROM plugin_nms_categories WHERE name = ?', array($name))) {
				nms_category_execute('INSERT INTO plugin_nms_categories (name, seed_key, sort_order, updated_at)
					VALUES (?, ?, ?, NOW())', array($name, $key, ++$order * 10));
			}
		}
...
/** Equipment category IDs are independent of graph_tree IDs. Zero means explicitly unclassified. */
function nms_category_exists($category_id) {
	return (int) $category_id > 0 && (int) db_fetch_cell_prepared(
		'SELECT COUNT(*) FROM plugin_nms_categories WHERE id = ?', array((int) $category_id)) === 1;
}

/** Read editable equipment categories in presentation order, not Cacti tree order. */
...
function nms_classification_text($text, $max_length) {
	if (!is_string($text)) throw new InvalidArgumentException('Classification text must be a string.');
	$text = trim((string) $text);
	if (!preg_match('//u', $text) || preg_match('/[\x00-\x1f\x7f]/', $text) ||
		preg_match_all('/./us', $text) > $max_length) {
		throw new InvalidArgumentException('Classification text contains invalid characters or exceeds its allowed length.');
	}
	return $text;
}
...
function nms_category_save($id, $name, $description) {
	...
	if ($name === '' || ($id > 0 && !nms_category_exists($id))) throw new InvalidArgumentException('Select a valid category and enter its name.');
	if ($id > 0) {
		nms_category_execute('UPDATE plugin_nms_categories SET name = ?, description = ?, updated_by = ?, updated_at = NOW() WHERE id = ?',
			array($name, $description, nms_current_user_id(), $id));
	} else {
		nms_category_execute('INSERT INTO plugin_nms_categories (name, description, updated_by, updated_at) VALUES (?, ?, ?, NOW())',
...
function nms_assign_template_category($template_id, $category_id) {
	if (!(int) db_fetch_cell_prepared('SELECT COUNT(*) FROM host_template WHERE id = ?', array((int) $template_id))) {
	...
	nms_category_execute('INSERT INTO plugin_nms_category_templates (host_template_id, category_id, assigned_at)
		VALUES (?, ?, NOW()) ON DUPLICATE KEY UPDATE category_id = VALUES(category_id), assigned_at = NOW()',
		array((int) $template_id, (int) $category_id));
}
...
function nms_new_device_category($choice, $template_id) {
	if ($choice === 'template') {
		$choice = db_fetch_cell_prepared('SELECT category_id FROM plugin_nms_category_templates WHERE host_template_id = ?', array((int) $template_id));
		if (!$choice) throw new InvalidArgumentException('This template has no equipment category suggestion. Select a category or Unclassified.');
	}
	if (!is_scalar($choice) || !preg_match('/^[0-9]+$/D', (string) $choice)) {
		throw new InvalidArgumentException('Select an equipment category or Unclassified.');
	}
	$category_id = (int) $choice;
	if ($category_id > 0 && !nms_category_exists($category_id)) throw new InvalidArgumentException('The equipment category no longer exists.');
	return $category_id;
}
...
function nms_device_classification_save($host_id, $category_id, $type, $role) {
	...
	if ($category_id < 0 || ($category_id > 0 && !nms_category_exists($category_id))) {
		throw new InvalidArgumentException('Select a valid equipment category or Unclassified.');
	}
	...
	nms_category_execute("INSERT INTO plugin_nms_device_classification
		...
		updated_by = VALUES(updated_by), updated_at = NOW()", array($host_id, $category_id, $type, $role, nms_current_user_id()));
}
