...
$tree_id = isset_request_var('tree_id') ? get_filter_request_var('tree_id') : (count($trees) ? (int) $trees[0]['id'] : 0);
$allowed_tree_ids = array_map('intval', array_column($trees, 'id'));
if ($tree_id > 0 && !in_array($tree_id, $allowed_tree_ids, true)) { http_response_code(403); die('This Cacti graph tree is not available.'); }
$selected_tree = null;
foreach ($trees as $tree) if ((int) $tree['id'] === $tree_id) $selected_tree = $tree;
$seen = array();
$tree_nodes = $tree_id > 0 ? nms_graph_tree_nodes($tree_id, 0, 0, $seen) : array();
...
require($config['base_path'] . '/plugins/nms/templates/graphs.php');
