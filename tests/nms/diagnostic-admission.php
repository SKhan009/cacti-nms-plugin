...
$mode='ready';$inserts=0;$released=0;$allowed='ping';
...
foreach (array_keys(nms_diag_labels()) as $tool) {
 ...
 foreach (['missing','offline','busy'] as $mode) {
  $before=$inserts;$rejected=false;
  try { nms_diag_run(1,$tool); } catch (RuntimeException $e) { $rejected=true; }
  if (!$rejected || $inserts!==$before) throw new Exception('New mode admission must fail closed: '.$tool);
 }
 $mode='ready';$before=$inserts;
 if (nms_diag_run(1,$tool)!==12 || $inserts!==$before+1) throw new Exception('New mode not admitted: '.$tool);
 $allowed='ping';$before=$inserts;$rejected=false;
 try { nms_diag_run(1,$tool); } catch (RuntimeException $e) { $rejected=true; }
