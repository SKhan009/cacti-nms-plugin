...
$nms_extra_css = 'css/nms-fault-config.css?v=2.0.0';
...
<main class="nms-shell nms-config-shell">
	...
	<?php } else { ?>
	<section class="nms-panel nms-config-panel"><div class="nms-config-bar"><div><h2>Cacti template mapping</h2><p>A mapping applies category fault rules to every current and future device using that Cacti host template.</p></div></div><div class="nms-category-strip"><?php foreach ($categories as $category) { ?><span><strong><?php print nms_h($category['name']); ?></strong><?php print (int) $category['template_count']; ?> templates · <?php print (int) $category['device_count']; ?> devices</span><?php } ?></div><div class="nms-table-wrap"><table class="nms-table nms-config-table"><thead><tr><th>Cacti host template</th><th>ID</th><th>Linked devices</th><th>Device category</th><th></th></tr></thead><tbody>
