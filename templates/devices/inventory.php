...
<section class="nms-panel">
	...
	<div class="nms-table-wrap nms-fit-table-wrap">
		<table class="nms-table nms-device-table">
			...
			?>
			<tr>
				<td><div class="nms-incident"><i class="nms-severity <?php print $status === 'Up' ? 'healthy' : 'critical'; ?>"></i><div><strong><?php print nms_h($device['description']); ?></strong><small><?php print nms_h($device['hostname']); ?></small><small><?php print nms_h($device['snmp_sysName'] ?: 'SNMP identity pending'); ?></small><?php if ($device['serial_status'] === 'failed') { ?><small>Serial number: live SNMP read failed</small><?php } elseif ($device['serial_number'] !== null && $device['serial_number'] !== '') { ?><small>Serial number: <?php print nms_h($device['serial_number']); ?><?php print $device['serial_status'] === 'changed' ? ' (changed)' : ''; ?></small><?php } elseif (!empty($device['serial_configured'])) { ?><small>Serial number: pending live poll</small><?php } ?></div></div></td>
