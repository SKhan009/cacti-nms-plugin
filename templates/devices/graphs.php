...
<section class="nms-panel nms-standalone-graph-builder" id="graph-builder">
	...
	<form class="nms-graph-builder-form" method="post" action="devices.php?tab=graphs#graph-builder">
		...
		<label class="source"><span>Cacti data-template item</span><select required name="data_template_rrd_id" <?php print !$source_item_count ? 'disabled' : ''; ?>><option value=""><?php print $source_item_count ? 'Select a reusable reading from Cacti' : 'Cacti has no reusable data-template items'; ?></option><?php foreach ($graph_data_template_items as $source_item) { ?><option value="<?php print (int) $source_item['data_template_rrd_id']; ?>"><?php print nms_h($source_item['data_template_name'] . ' — ' . $source_item['data_source_name'] . ((int) $source_item['graph_template_count'] > 0 ? ' — used by ' . (int) $source_item['graph_template_count'] . ' template(s)' : ' — not yet graphed')); ?></option><?php } ?></select><small>This is a global template item, not a reading from one device.</small></label>
