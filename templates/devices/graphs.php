...
<section class="nms-panel nms-standalone-graph-builder" id="graph-builder">
	...
	<form class="nms-graph-builder-form" method="post" action="templates.php?section=graph&amp;view=builder#graph-builder">
		...
		<section class="nms-graph-form-section">
			...
			<div class="nms-graph-section-grid">
				...
				<label><span>Graph item style</span><select id="nmsGraphItemStyle" name="graph_style"><?php foreach ($graph_item_types as $item_type_name) { ?><option value="<?php print nms_h($item_type_name); ?>" <?php print $item_type_name === ($graph_item_types[$struct_graph_item['graph_type_id']['default']] ?? '') ? 'selected' : ''; ?>><?php print nms_h($item_type_name); ?></option><?php } ?></select><small id="nmsGraphItemHelp">Choose any graph item type supported by Cacti.</small></label>
				<?php nms_core_render_field('consolidation_function_id', $struct_graph_item['consolidation_function_id'], $struct_graph_item['consolidation_function_id']['default']); ?>
				<label><span>Item color</span><select id="nmsGraphColor" name="color_id" class="nms-search-select" data-search-placeholder="Search colors by name or hex"><option value="0">None</option><?php foreach ($graph_colors as $color) { ?><option value="<?php print (int) $color['id']; ?>" data-hex="<?php print nms_h($color['hex']); ?>" <?php print (int) $color['id'] === (int) $struct_graph_item['color_id']['default'] ? 'selected' : ''; ?>><?php print nms_h($color['name']); ?></option><?php } ?></select></label>
				...
				<?php nms_core_render_field('gprint_id', $struct_graph_item['gprint_id'], $struct_graph_item['gprint_id']['default']); ?>
				...
				<label data-item-field="stack_source_id" hidden><span>Stack base data-template item</span><select name="stack_source_id" class="nms-search-select" data-search-placeholder="Search stack base readings"><option value="">Select the reading to draw below the stack</option><?php foreach ($graph_data_template_items as $source_item) { ?><option value="<?php print (int) $source_item['data_template_rrd_id']; ?>"><?php print nms_h($source_item['data_template_name'] . ' — ' . $source_item['data_source_name']); ?></option><?php } ?></select><small>A base item is created first, followed by the selected stacked item.</small></label>
				<?php nms_core_render_field('vdef_id', $struct_graph_item['vdef_id'], $struct_graph_item['vdef_id']['default']); ?>
				<?php nms_core_render_field('text_format', $struct_graph_item['text_format'], $struct_graph_item['text_format']['default']); ?>
				...
				<?php nms_core_render_field('line_width', $struct_graph_item['line_width'], $struct_graph_item['line_width']['default']); ?>
				<?php nms_core_render_field('dashes', $struct_graph_item['dashes'], $struct_graph_item['dashes']['default']); ?>
				<?php nms_core_render_field('dash_offset', $struct_graph_item['dash_offset'], $struct_graph_item['dash_offset']['default']); ?>
				<?php nms_core_render_field('textalign', $struct_graph_item['textalign'], $struct_graph_item['textalign']['default']); ?>
		...
		<section class="nms-graph-form-section">
			...
			<div class="nms-graph-section-grid">
				<label><span>Graph template name <em>optional</em></span><input name="graph_name" maxlength="<?php print (int) $fields_graph_template_template_edit['name']['max_length']; ?>" placeholder="Automatic from data template"></label>
				<?php foreach (array('multiple', 'test_source') as $field_name) {
					$field = $fields_graph_template_template_edit[$field_name];
					nms_core_render_field($field_name, $field, isset_request_var($field_name) ? get_nfilter_request_var($field_name) : ($field['default'] ?? ''));
				} ?>
			</div>
		</section>
		<?php
		...
		$section_open = false;
		$submitted_graph = ($_SERVER['REQUEST_METHOD'] === 'POST' && get_nfilter_request_var('nms_action') === 'create_graph_template');
		foreach ($struct_graph as $field_name => $field) {
			if ($field['method'] === 'spacer') {
				if ($section_open) print '</div></section>';
				print '<section class="nms-graph-form-section"><h3>' . nms_h($field['friendly_name']) . '</h3><div class="nms-graph-section-grid">';
				$section_open = true;
				continue;
			}
			$value = $submitted_graph ? (isset_request_var($field_name) ? get_nfilter_request_var($field_name) : '') : ($field['default'] ?? '');
		...
		if ($section_open) print '</div></section>';
		?>
		<div class="nms-graph-submit"><button type="submit" <?php print !$source_item_count ? 'disabled' : ''; ?>>Create graph template in Cacti</button></div>
	</form>
</section>
	...
	<div class="nms-association-table nms-global-graph-table">
		...
		<?php if (!$global_graph_templates) { ?><div class="nms-association-empty">Cacti has no graph templates.</div><?php } ?>
			<?php foreach ($global_graph_templates as $graph_template) { ?><div class="nms-association-row"><strong><?php print nms_h($graph_template['name']); ?></strong><span><?php print (int) $graph_template['id']; ?></span><span><?php print (int) $graph_template['graph_count']; ?></span><span><?php print (int) $graph_template['width']; ?> × <?php print (int) $graph_template['height']; ?></span><span><?php print nms_h($image_types[$graph_template['image_format_id']] ?? 'Unknown core format'); ?></span><span><?php print nms_h($graph_template['vertical_label'] ?: 'Not set'); ?></span><span class="nms-template-actions"><a href="<?php print nms_h('templates.php?section=graph&core=' . rawurlencode('graph_templates.php?action=template_edit&id=' . (int) $graph_template['id'])); ?>">Open</a><?php if (!empty($graph_template['nms_deletable']) && (int) $graph_template['graph_count'] === 0) { ?><form method="post" action="templates.php?section=graph&amp;view=builder#graph-template-list" onsubmit="return confirm('Permanently delete this unused NMS-created graph template?');"><input type="hidden" name="__csrf_magic" value="<?php print nms_h($nms_csrf_token); ?>"><input type="hidden" name="nms_action" value="delete_graph_template"><input type="hidden" name="graph_template_id" value="<?php print (int) $graph_template['id']; ?>"><button class="nms-delete-x" type="submit" aria-label="Delete <?php print nms_h($graph_template['name']); ?>" title="Delete unused NMS-created template">×</button></form><?php } ?></span></div><?php } ?>
	</div>
</section>
