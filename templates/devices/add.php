...
$device_form_is_prefilled = !$device_form_is_edit && (!empty($snmpsim_import_id) || isset_request_var('host_template_id') || isset_request_var('snmp_community') || isset_request_var('snmp_port'));
...
<section class="nms-panel nms-form-panel">
	...
	<form id="nms-device-form" method="post" action="devices.php?tab=<?php print $device_form_is_edit ? 'edit&id=' . (int) $device_values['id'] : 'add'; ?>" class="nms-device-form">
		...
		<fieldset><legend>Device identity</legend><div class="nms-form-grid">
			...
			<label><span>Serial number (manual)</span><input id="nmsManualSerial" name="manual_serial_number" maxlength="191" value="<?php print nms_h(is_string($manual_serial_value) ? $manual_serial_value : ''); ?>" placeholder="Serial printed on the device"><small>Optional; enter manually when no SNMP serial is available.</small></label>
		...
		<fieldset class="nms-snmp-fieldset"><legend>SNMP connection</legend>
			...
			<label><span>SNMP version</span><select id="nmsSnmpVersion" name="snmp_version"><?php foreach ($snmp_versions as $value => $label) { ?><option value="<?php print nms_h($value); ?>" <?php print (string) $selected_snmp_version === (string) $value ? 'selected' : ''; ?>><?php print nms_h($label); ?></option><?php } ?></select></label>
			...
			<label class="nms-conditional-field" data-snmp-versions="3"><span>SNMP v3 authentication</span><select id="nmsSnmpAuthProtocol" name="snmp_auth_protocol"><?php foreach ($snmp_auth_protocols as $value => $label) { ?><option value="<?php print nms_h($value); ?>" <?php print (string) $selected_auth_protocol === (string) $value ? 'selected' : ''; ?>><?php print nms_h($label); ?></option><?php } ?></select></label>
			...
			<label class="nms-conditional-field" data-snmp-versions="3" data-snmp-auth-enabled="1"><span>SNMP v3 privacy</span><select id="nmsSnmpPrivProtocol" name="snmp_priv_protocol"><?php foreach ($snmp_priv_protocols as $value => $label) { ?><option value="<?php print nms_h($value); ?>" <?php print (string) $selected_priv_protocol === (string) $value ? 'selected' : ''; ?>><?php print nms_h($label); ?></option><?php } ?></select></label>
		...
		<fieldset><legend>Availability and polling</legend><div class="nms-form-grid">
			<label><span>Availability method</span><select name="availability_method"><?php foreach ($availability_options as $value => $label) { ?><option value="<?php print nms_h($value); ?>" <?php print (string) $selected_availability === (string) $value ? 'selected' : ''; ?>><?php print nms_h($label); ?></option><?php } ?></select></label>
			<label><span>Ping method</span><select name="ping_method"><?php foreach ($ping_methods as $value => $label) { ?><option value="<?php print nms_h($value); ?>" <?php print (string) $selected_ping_method === (string) $value ? 'selected' : ''; ?>><?php print nms_h($label); ?></option><?php } ?></select></label>
			<label><span>Ping port</span><input type="number" name="ping_port" min="0" max="65535" step="1" value="<?php print (int) $selected_ping_port; ?>"></label>
			<label><span>Ping timeout (ms)</span><input type="number" name="ping_timeout" min="1" step="1" value="<?php print (int) $selected_ping_timeout; ?>"></label>
			<label><span>Ping retries</span><input type="number" name="ping_retries" min="0" step="1" value="<?php print (int) $selected_ping_retries; ?>"></label>
