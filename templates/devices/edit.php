...
<div class="nms-device-associations">
	<section class="nms-panel" id="graph-templates">
		...
		<form class="nms-association-add" method="post" action="devices.php?tab=edit&id=<?php print $device_id; ?>#graph-templates">
	...
	<section class="nms-panel" id="data-queries">
		...
		<form class="nms-association-add query" method="post" action="devices.php?tab=edit&id=<?php print $device_id; ?>#data-queries">
			<input type="hidden" name="__csrf_magic" value="<?php print nms_h($nms_csrf_token); ?>">
			<input type="hidden" name="nms_action" value="add_data_query">
			<input type="hidden" name="id" value="<?php print $device_id; ?>">
			<label><span>Add Data Query</span><select required name="snmp_query_id" <?php print !$available_data_queries ? 'disabled' : ''; ?>><option value=""><?php print $available_data_queries ? 'Select a Cacti data query' : 'All available queries are associated'; ?></option><?php foreach ($available_data_queries as $available_query) { ?><option value="<?php print (int) $available_query['id']; ?>"><?php print nms_h($available_query['name']); ?></option><?php } ?></select></label>
			...
			<button type="submit" <?php print !$available_data_queries ? 'disabled' : ''; ?>>Add</button>
		</form>
	</section>
</div>
