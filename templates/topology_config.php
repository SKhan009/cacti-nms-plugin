...
<main class="nms-shell nms-topology-config">
 ...
 <section class="nms-panel"><div class="nms-panel-head"><div><h2>Physical port capacity</h2><p>Category defaults with optional device-type profiles such as Switch, Router or UPS. These are configured counts, not discovered interfaces or live port status.</p></div></div>
  <form method="post" action="<?php print nms_h($base_url); ?>" class="nms-config-form" data-confirm="Save this physical-port profile? It will apply to matching classified devices.">
  ...
  <div class="nms-table-wrap"><table class="nms-table"><thead><tr><th>Category</th><th>Device type</th><th>Physical ports</th><th>Action</th></tr></thead><tbody>
 ...
 <section class="nms-panel"><div class="nms-panel-head"><div><h2><?php print $node ? 'Edit node / vehicle' : 'Create node / vehicle'; ?></h2><p>Each node or vehicle has its own racks. Units are numbered from U1 at the bottom. Counts are editable, not fixed to 4, 5 or 8.</p></div></div>
  <form method="post" action="<?php print nms_h($base_url); ?>" class="nms-config-form" data-confirm="Save this node and rack count? Reducing the count removes only empty racks at the end; occupied racks are protected.">
 ...
 <section class="nms-panel"><div class="nms-panel-head"><div><h2>Device rack placement</h2><p>One physical placement per Cacti device. Saving an existing device moves it; overlapping units are rejected.</p></div></div>
  <form class="nms-config-form" method="post" action="<?php print nms_h($base_url); ?>" data-confirm="Save this device placement? An existing placement for this device will move to the selected rack and units."><?php nms_topology_form_fields('place_device'); ?>
 ...
 <?php } ?></div></section>
 <?php } ?>
 <section class="nms-panel"><div class="nms-panel-head"><h2>Devices and configured physical capacity</h2></div><div class="nms-table-wrap"><table class="nms-table"><thead><tr><th>Device</th><th>Category / type</th><th>Configured physical ports</th><th>Rack / units</th><th>Action</th></tr></thead><tbody>
