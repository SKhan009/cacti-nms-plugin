...
function nms_rule_scope_defaults()
{
	return [
		"scope_kind" => "all",
		"scope_host_template_id" => 0,
		"scope_device_type" => "",
		"scope_sys_object_id" => "",
		"scope_data_input_id" => 0,
		"scope_snmp_version" => "any",
		"scope_host_id" => 0,
		"scope_local_data_id" => 0,
		"scope_query_id" => 0,
		"scope_index" => "",
	];
}
...
{
	$columns = [
		"scope_kind" => "VARCHAR(16) NOT NULL DEFAULT 'all'",
...
function nms_rule_scope_validate($input, $rule)
{
	...
	foreach (nms_rule_scope_defaults() as $name => $default) {
		if (!array_key_exists($name, $input)) {
			throw new InvalidArgumentException("Submit every rule applicability field.");
		}
		$limit = $name === "scope_device_type" ? 150 : ($name === "scope_sys_object_id" ? 128 : 191);
		$values[$name] = is_int($default)
			? nms_rule_scope_id($input[$name])
			: nms_classification_text($input[$name], $limit);
	}
	if (!in_array($values["scope_kind"], ["all", "device", "interface", "component", "service"], true)) {
		throw new InvalidArgumentException("Select a supported target scope.");
	}
	foreach (
		[
			"scope_host_template_id" => "host_template",
			"scope_data_input_id" => "data_input",
			"scope_query_id" => "snmp_query",
		]
		as $key => $table
	) {
		if (
			$values[$key] &&
			!(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM " . $table . " WHERE id = ?", [$values[$key]])
		) {
	...
	if (
		$version !== "any" &&
		(!is_array($snmp_versions) || !array_key_exists($version, $snmp_versions) || (int) $version === 0)
	) {
	...
	if ($rule["metric"] !== "parameter") {
		if (
			!in_array($values["scope_kind"], ["all", "device"], true) ||
			$values["scope_data_input_id"] ||
			$values["scope_local_data_id"] ||
			$values["scope_query_id"] ||
			$values["scope_index"] !== ""
		) {
			throw new InvalidArgumentException(
				"This device-level rule does not have a native indexed data-source target.",
			);
		}
		if ($rule["metric"] === "core_status" && $version !== "any") {
			throw new InvalidArgumentException(
				"Core availability follows the native availability method; an SNMP credential setting alone does not prove SNMP is used.",
			);
		}
	}
	if ($values["scope_host_id"]) {
		...
		if (
			!(int) db_fetch_cell_prepared(
				"SELECT COUNT(*) FROM host AS h
			INNER JOIN plugin_nms_device_classification AS c ON c.host_id = h.id
			WHERE h.id = ? AND h.deleted = '' AND c.category_id = ?",
				[$values["scope_host_id"], $rule["category_id"]],
			)
		) {
			throw new InvalidArgumentException("Select a device assigned to this device segment.");
		}
	}
	if ($values["scope_index"] !== "" && (!$values["scope_host_id"] || !$values["scope_query_id"])) {
		throw new InvalidArgumentException(
			"An index needs both its native device and data query; it is not a TCP/UDP port.",
		);
	}
	...
	if (in_array($values["scope_kind"], ["component", "service"], true) && !$values["scope_local_data_id"]) {
		throw new InvalidArgumentException(
			"Select an exact native data source for a reviewed component or service; NMS cannot infer that role from a name.",
		);
	}
	if ($values["scope_local_data_id"]) {
		if (!$values["scope_host_id"]) {
			throw new InvalidArgumentException("A local data source requires its native device.");
		}
		if (
			!preg_match('/^dtrr:([1-9][0-9]*)$/D', $rule["parameter_key"], $match) ||
			!(int) db_fetch_cell_prepared(
				'SELECT COUNT(*) FROM data_local AS dl
			...
			WHERE dl.id = ? AND dl.host_id = ? AND dtr.local_data_template_rrd_id = ?',
				[$values["scope_local_data_id"], $values["scope_host_id"], (int) $match[1]],
			)
		) {
			throw new InvalidArgumentException(
				"The selected data source does not provide this rule parameter on this device.",
			);
		}
	}
	if (
		$values["scope_index"] !== "" &&
		!(int) db_fetch_cell_prepared(
			'SELECT COUNT(*) FROM host_snmp_cache
		WHERE host_id = ? AND snmp_query_id = ? AND snmp_index = ?',
			[$values["scope_host_id"], $values["scope_query_id"], $values["scope_index"]],
		)
	) {
...
function nms_rule_scope_save($category_id, $rule_id, $input)
{
	...
	$rule = db_fetch_row_prepared("SELECT * FROM plugin_nms_fault_rules WHERE id = ? AND category_id = ?", [
		(int) $rule_id,
		(int) $category_id,
	]);
	if (!$rule) {
		throw new InvalidArgumentException("Select an existing rule in this category.");
	}
	...
	nms_category_execute(
		"UPDATE plugin_nms_fault_rules SET " .
			implode(", ", $sets) .
			", updated_at = NOW() WHERE id = ? AND category_id = ?",
		array_merge(array_values($values), [(int) $rule_id, (int) $category_id]),
	);
}
...
function nms_require_category_policy_access($category_id)
{
	...
	if (
		(int) db_fetch_cell_prepared(
			"SELECT COUNT(*) FROM host AS h
		INNER JOIN plugin_nms_device_classification AS c ON c.host_id = h.id
		WHERE c.category_id = ? AND h.deleted = '' AND NOT ($visible)",
			[(int) $category_id],
		)
	) {
		throw new RuntimeException(
			"This category includes devices outside your Cacti permissions. Ask an administrator with access to the complete category to change shared policy.",
		);
	}
}
...
function nms_rule_scope_matches($rule, $context)
{
	foreach (array_keys(nms_rule_scope_defaults()) as $key) {
		if (!array_key_exists($key, $rule)) {
			throw new RuntimeException("NMS rule applicability schema is incomplete; run the reviewed plugin upgrade.");
		}
	}
	foreach (
		[
			"scope_host_template_id" => "host_template_id",
			"scope_data_input_id" => "data_input_id",
			"scope_host_id" => "host_id",
			"scope_local_data_id" => "local_data_id",
			"scope_query_id" => "snmp_query_id",
		]
		as $key => $native
	) {
		if ((int) $rule[$key] && (int) $rule[$key] !== (int) ($context[$native] ?? 0)) {
			return false;
		}
	}
	if ($rule["scope_device_type"] !== "" && $rule["scope_device_type"] !== ($context["device_type"] ?? "")) {
		return false;
	}
	if ($rule["scope_sys_object_id"] !== "" && $rule["scope_sys_object_id"] !== ($context["snmp_sysObjectID"] ?? "")) {
		return false;
	}
	if ($rule["scope_index"] !== "" && $rule["scope_index"] !== (string) ($context["snmp_index"] ?? "")) {
		return false;
	}
	if ($rule["scope_snmp_version"] !== "any") {
		$is_snmp = $rule["metric"] === "inventory_status";
		if ($rule["metric"] === "parameter") {
			if (!defined("DATA_INPUT_TYPE_SNMP") || !defined("DATA_INPUT_TYPE_SNMP_QUERY")) {
				throw new RuntimeException("Cacti SNMP input-type definitions are unavailable.");
			}
			$is_snmp = in_array(
				(int) ($context["input_type_id"] ?? 0),
				[DATA_INPUT_TYPE_SNMP, DATA_INPUT_TYPE_SNMP_QUERY],
				true,
			);
		}
		if (!$is_snmp || (string) $rule["scope_snmp_version"] !== (string) ($context["snmp_version"] ?? "")) {
			return false;
		}
	}
	if ($rule["scope_kind"] === "interface") {
		return !empty($context["is_interface"]);
	}
	if ($rule["scope_kind"] === "device") {
		return empty($context["snmp_query_id"]) && (string) ($context["snmp_index"] ?? "") === "";
	}
	if (in_array($rule["scope_kind"], ["component", "service"], true)) {
		return (int) $rule["scope_local_data_id"] > 0;
	}
	if ($rule["scope_kind"] !== "all") {
		throw new RuntimeException("Unknown persisted rule scope; no default scope was substituted.");
	}
	return true;
}
