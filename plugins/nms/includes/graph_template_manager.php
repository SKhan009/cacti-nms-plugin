...
function nms_graph_template_create($data_template_rrd_id, $graph_name, $vertical_label, $options = [])
{
	global $graph_item_types, $struct_graph_item, $fields_graph_template_template_edit;
	if (!is_scalar($graph_name)) {
		throw new InvalidArgumentException("Enter a graph template name.");
	}
	$data_template_rrd_id = (int) $data_template_rrd_id;
	$source = db_fetch_row_prepared(
		'SELECT dt.id AS data_template_id, dt.name AS data_template_name,
		...
		WHERE dtr.id = ? AND dtr.local_data_id = 0',
		[$data_template_rrd_id],
	);
	if (!$source) {
		throw new InvalidArgumentException("Select a reusable Cacti data-template item.");
	}

	$name_length = (int) $fields_graph_template_template_edit["name"]["max_length"];
	$default_name = mb_substr(
		"NMS " . $source["data_template_name"] . " - " . $source["data_source_name"],
		0,
		$name_length,
	);
	if (trim((string) $graph_name) === "") {
		...
		$suffix = 2;
		while ((int) db_fetch_cell_prepared("SELECT COUNT(*) FROM graph_templates WHERE name = ?", [$graph_name])) {
			$ending = " " . $suffix++;
	...
	if (!array_key_exists("title", $options)) {
		$options["title"] = "|host_description| - " . $graph_name;
	}
	...
	if ((int) db_fetch_cell_prepared("SELECT COUNT(*) FROM graph_templates WHERE name = ?", [$graph_name])) {
		throw new InvalidArgumentException(
			"A Cacti graph template with this name already exists. Choose another name.",
		);
	}

	// Native defaults and database choices must not silently turn into fixed IDs.
	foreach (["color_id", "cdef_id", "gprint_id", "vdef_id"] as $key) {
		...
		if ($key === "color_id") {
			if (
				...
				((int) $value && !db_fetch_cell_prepared("SELECT id FROM colors WHERE id = ?", [$value]))
			) {
				throw new InvalidArgumentException("Select a color from Cacti.");
			}
		} else {
	...
	$options["alpha"] = nms_core_field_value(
		"alpha",
		$struct_graph_item["alpha"],
		$options["alpha"] ?? $struct_graph_item["alpha"]["default"],
	);
	if (isset($options["consolidation_function_id"])) {
		$options["consolidation_function_id"] = nms_core_field_value(
			"consolidation_function_id",
			$struct_graph_item["consolidation_function_id"],
			$options["consolidation_function_id"],
		);
	}
	$item_rows = nms_graph_item_rows($graph_item_types, $data_template_rrd_id, $source["data_source_name"], $options);
	foreach ($item_rows as $item_row) {
		if (
			$item_row["task_item_id"] &&
			!(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM data_template_rrd WHERE id = ? AND local_data_id = 0", [
				$item_row["task_item_id"],
			])
		) {
	...
	try {
		$parent = ["id" => 0, "hash" => get_hash_graph_template(0), "name" => $graph_name];
		foreach (["multiple", "test_source"] as $key) {
			$parent[$key] = nms_core_field_value(
				$key,
				$fields_graph_template_template_edit[$key],
				$options[$key] ?? "",
			);
		}
		$graph_template_id = (int) sql_save($parent, "graph_templates");
		if (!$graph_template_id) {
			throw new RuntimeException("Cacti could not create the graph template.");
		}
		...
		if (!sql_save($graph_values, "graph_templates_graph")) {
			throw new RuntimeException("Cacti could not save graph options.");
		}
		$seed = ["graph_template_id" => $graph_template_id, "local_graph_id" => 0, "local_graph_template_item_id" => 0];
		$input_ids = [];
		foreach ($item_rows as $index => $item_row) {
			$item = array_merge($seed, $item_row, [
				"id" => 0,
				"hash" => get_hash_graph_template(0, "graph_template_item"),
				"sequence" => $index + 1,
			]);
			$item_id = (int) sql_save($item, "graph_templates_item");
			if (!$item_id) {
				throw new RuntimeException("Cacti could not save the graph item.");
			}
			...
			if (!isset($input_ids[$task_id])) {
				$ds_name = db_fetch_cell_prepared("SELECT data_source_name FROM data_template_rrd WHERE id = ?", [
					$task_id,
				]);
				$input_ids[$task_id] = (int) sql_save(
					[
						"id" => 0,
						"hash" => get_hash_graph_template(0, "graph_template_input"),
						"graph_template_id" => $graph_template_id,
						"name" => "Data Source [" . $ds_name . "]",
						"column_name" => "task_item_id",
					],
					"graph_template_input",
				);
				if (!$input_ids[$task_id]) {
					throw new RuntimeException("Cacti could not save the graph input.");
				}
			}
			db_execute_prepared(
				"INSERT INTO graph_template_input_defs (graph_template_input_id, graph_template_item_id) VALUES (?, ?)",
				[$input_ids[$task_id], $item_id],
			);
		}
...
function nms_graph_template_delete($graph_template_id)
{
	...
	if (
		(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM graph_local WHERE graph_template_id = ?", [
			$graph_template_id,
		]) > 0
	) {
		throw new InvalidArgumentException(
			"This graph template is in use. Remove its device graphs before deleting it.",
		);
	}
	$input_ids = db_fetch_assoc_prepared("SELECT id FROM graph_template_input WHERE graph_template_id = ?", [
		$graph_template_id,
	]);
	foreach ($input_ids as $input) {
		db_execute_prepared("DELETE FROM graph_template_input_defs WHERE graph_template_input_id = ?", [
			(int) $input["id"],
		]);
	}
	foreach (
		["graph_template_input", "graph_templates_graph", "graph_templates_item", "host_template_graph", "host_graph"]
		as $table
	) {
		db_execute_prepared("DELETE FROM " . $table . " WHERE graph_template_id = ?", [$graph_template_id]);
	}
	db_execute_prepared("UPDATE plugin_nms_snmprec_oids SET graph_template_id = 0 WHERE graph_template_id = ?", [
		$graph_template_id,
	]);
	db_execute_prepared("DELETE FROM graph_templates WHERE id = ?", [$graph_template_id]);
