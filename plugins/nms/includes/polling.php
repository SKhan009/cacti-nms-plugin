...
function nms_poller_bottom()
{
	try {
		...
		cacti_log(
			"NMS post-poll processing failed (" .
				get_class($error) .
				"). Check the NMS schema and Cacti database logs; Cacti polling continues.",
			false,
			"NMS",
		);
	}
...
function nms_poller_schema_ready()
{
	...
	if (!$warned) {
		cacti_log(
			"NMS collection skipped: database upgrade is incomplete. Back up the database and upgrade NMS through Cacti Plugin Management. Native Cacti polling is unchanged.",
			false,
			"NMS",
		);
	}
...
function nms_poller_output($rrd_update_array)
{
	try {
		if (nms_poller_schema_ready()) {
			nms_capture_poller_output($rrd_update_array);
		}
	} catch (Throwable $error) {
		cacti_log(
			"NMS reading capture failed (" .
				get_class($error) .
				"). Check the NMS schema and Cacti database logs; native RRD updates continue.",
			false,
			"NMS",
		);
	}
...
function nms_capture_poller_output($rrd_update_array)
{
	...
	foreach ($rrd_update_array as $rrd_path => $rrd_data) {
		$item = db_fetch_row_prepared(
			"SELECT pi.local_data_id, pi.host_id, dtd.data_template_id,
			dl.snmp_index,
		...
		foreach ($rrd_data["times"] as $sample_time => $fields) {
			...
			if (!is_numeric($sample_time) || (int) $sample_time <= 0 || (int) $sample_time > time()) {
				cacti_log(
					"Ignored NMS sample with an invalid collection timestamp for data source " .
						(int) $item["local_data_id"],
					false,
					"NMS",
				);
				continue;
			...
			foreach ($fields as $field => $value) {
				$definition = db_fetch_row_prepared(
					"SELECT dtr.local_data_template_rrd_id,
				...
				$parameter_key =
					$template_item_id > 0
						...
						: "dt:" .
							(int) $item["data_template_id"] .
							":" .
							preg_replace("/[^A-Za-z0-9_.-]/", "_", (string) $field);
				/*
				...
				nms_category_execute(
					"INSERT INTO plugin_nms_device_parameters
