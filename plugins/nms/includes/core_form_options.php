...
{
	global $struct_graph;
	if (empty($struct_graph)) {
		throw new RuntimeException("Cacti graph form definitions are unavailable.");
	}
	$values = [];
	foreach ($struct_graph as $name => $field) {
		...
		$pattern = null;
		if (in_array($name, ["width", "height", "base_value", "unit_length", "tab_width"], true)) {
			$pattern = '/^[0-9]+$/D';
		}
		if ($name === "unit_exponent_value") {
			$pattern = '/^-?[0-9]+$/D';
		}
		if (in_array($name, ["lower_limit", "upper_limit"], true)) {
			$pattern = '/^(?:-?(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)(?:[eE][+-]?[0-9]+)?|U)$/D';
		}
		if ($name === "right_axis") {
			$pattern = '/^-?(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+):-?(?:[0-9]+(?:\.[0-9]*)?|\.[0-9]+)$/D';
		}
		if ($value !== "" && $pattern && !preg_match($pattern, $value)) {
			throw new InvalidArgumentException("Invalid Cacti graph value for " . $name . ".");
		}
		if ($value === "" && in_array($name, ["title", "width", "height", "base_value"], true)) {
...
function nms_core_graph_request()
{
	...
	foreach (array_merge($struct_graph, $fields_graph_template_template_edit) as $name => $field) {
		if (in_array($field["method"], ["spacer", "hidden", "hidden_zero"], true) || $name === "name") {
			continue;
		}
		$values[$name] = isset_request_var($name) ? get_nfilter_request_var($name) : "";
		if (isset($struct_graph[$name])) {
			$values["t_" . $name] = "";
		}
	}
	return $values;
...
function nms_core_render_field($name, $field, $value, $override = null)
{
	$method = $field["method"];
	$id = "nmsCore_" . $name;
	print '<div class="nms-core-field"><label for="' .
		nms_h($id) .
		'"><span>' .
		nms_h(html_entity_decode($field["friendly_name"], ENT_QUOTES, "UTF-8")) .
		"</span></label>";
	if (in_array($method, ["drop_array", "drop_sql", "radio"], true)) {
		print '<select id="' . nms_h($id) . '" name="' . nms_h($name) . '">';
		foreach (nms_core_field_choices($field) as $key => $caption) {
			print '<option value="' .
				nms_h($key) .
				'"' .
		...
		print '<input id="' .
			...
			nms_h($value) .
			'"' .
			(isset($field["max_length"]) ? ' maxlength="' . (int) $field["max_length"] . '"' : "") .
			">";
	} else {
		throw new RuntimeException(
			"Unsupported Cacti field method: " . $method . ". Use the native editor for this version.",
		);
	}
	if (!empty($field["description"])) {
		print "<small>" .
			nms_h(
				html_entity_decode(
					strip_tags(str_replace(["<br>", "<br/>"], " ", $field["description"])),
					ENT_QUOTES,
					"UTF-8",
				),
			) .
