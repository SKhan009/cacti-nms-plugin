...
/** Explicit future-device rules use core template/site/collector IDs, never copied inventory. */
function nms_nd_rule_save($input)
{
	nms_require_management();
	$name = $input["rule_name"] ?? "";
	if (!is_string($name) || trim($name) === "" || strlen($name) > 100 || preg_match('/[\x00-\x1f\x7f]/', $name)) {
		throw new InvalidArgumentException("Enter a rule name of 1–100 bytes.");
	}
	$values = [trim($name)];
	foreach (
		[
			"preset_id" => "plugin_nms_discovery_presets",
			"site_id" => "sites",
			"host_template_id" => "host_template",
			"poller_id" => "poller",
		]
		as $key => $table
	) {
		$id = nms_topology_integer($input[$key] ?? 0, 1, 2147483647, $key);
		if (!db_fetch_cell_prepared("SELECT id FROM " . $table . " WHERE id=?", [$id])) {
			throw new InvalidArgumentException("Select an existing " . $key . ".");
		}
		$values[] = $id;
	}
	$values[] = (int) db_fetch_cell("SELECT COALESCE(MAX(id),0) FROM host");
	$values[] = nms_current_user_id();
	nms_category_execute(
		"INSERT INTO plugin_nms_discovery_rules(name,preset_id,site_id,host_template_id,poller_id,after_host_id,created_by,created_at) VALUES (?,?,?,?,?,?,?,NOW())",
		$values,
	);
}
/**
 * Handles nd apply rules.
 */
function nms_nd_apply_rules()
{
	global $config;
	require_once $config["base_path"] . "/lib/auth.php";
	$rules = db_fetch_assoc_prepared(
		"SELECT * FROM plugin_nms_discovery_rules WHERE enabled=1 AND poller_id=? ORDER BY id",
		[$config["poller_id"]],
	);
	foreach ($rules as $rule) {
		$user = (int) $rule["created_by"];
		if (
		...
		if (
			!db_fetch_cell_prepared("SELECT id FROM plugin_nms_discovery_presets WHERE id=? AND enabled=1", [
				$rule["preset_id"],
			])
		) {
			continue;
		...
		$hosts = db_fetch_assoc_prepared(
			"SELECT h.id FROM host h LEFT JOIN plugin_nms_discovery_devices d ON d.host_id=h.id WHERE h.id>? AND h.site_id=? AND h.host_template_id=? AND h.poller_id=? AND h.deleted='' AND h.disabled='' AND d.host_id IS NULL ORDER BY h.id LIMIT 500",
			[$rule["after_host_id"], $rule["site_id"], $rule["host_template_id"], $rule["poller_id"]],
		);
		foreach ($hosts as $host) {
			if (is_device_allowed($host["id"], $user)) {
				// First matching rule wins; explicit assignment/unassignment is never replaced.
				nms_category_execute(
					"INSERT IGNORE INTO plugin_nms_discovery_devices(host_id,preset_id,methods,collection_enabled) VALUES (?,?,'',1)",
					[$host["id"], $rule["preset_id"]],
				);
			}
		}
	}
