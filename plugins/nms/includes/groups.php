...
function nms_group_save($id, $name, $description)
{
	...
	if ($name === "") {
		throw new InvalidArgumentException("An operational group needs a name.");
	}
	if ($id > 0 && !(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM plugin_nms_groups WHERE id = ?", [$id])) {
		throw new InvalidArgumentException("Select an existing operational group.");
	}
	if (
		(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM plugin_nms_groups WHERE name = ? AND id != ?", [$name, $id])
	) {
		throw new InvalidArgumentException("An operational group with this name already exists.");
	}
	if ($id > 0) {
		nms_category_execute(
			"UPDATE plugin_nms_groups SET name = ?, description = ?, updated_by = ?, updated_at = NOW() WHERE id = ?",
			[$name, $description, nms_current_user_id(), $id],
		);
	} else {
		nms_category_execute(
			"INSERT INTO plugin_nms_groups (name, description, updated_by, updated_at) VALUES (?, ?, ?, NOW())",
...
function nms_group_membership_save($host_id, $group_ids)
{
	...
	if (!is_array($group_ids)) {
		throw new InvalidArgumentException("Submit a list of operational groups.");
	}
	...
	foreach ($group_ids as $id) {
		if (
			...
			!(int) db_fetch_cell_prepared("SELECT COUNT(*) FROM plugin_nms_groups WHERE id = ?", [(int) $id])
		) {
	...
	try {
		nms_category_execute("DELETE FROM plugin_nms_group_members WHERE host_id = ?", [$host_id]);
		foreach ($ids as $id) {
			nms_category_execute(
				"INSERT INTO plugin_nms_group_members (group_id, host_id, updated_by, updated_at) VALUES (?, ?, ?, NOW())",
				[$id, $host_id, nms_current_user_id()],
			);
		}
