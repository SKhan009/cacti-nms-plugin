	...
	this.sendData = function (data) {
		...
		// Otherwise, send as multiple instructions
			...
			for (
				...
				__send_blob(
					bytes.subarray(offset, offset + guac_writer.blobLength),
				);
		}
	};
	...
	var splitAudioPacket = function splitAudioPacket(data) {
		...
		return [
			new SampleArray(
				data.buffer.slice(
					0,
					optimalSplitLength * format.bytesPerSample,
				),
			),
			new SampleArray(
...
Guacamole.RawAudioRecorder = function RawAudioRecorder(stream, mimetype) {
	...
	var interpolateSample = function getValueAt(audioData, t) {
		...
		for (var i = start; i <= end; i++) {
			sum +=
				(audioData[i] || 0) * lanczos(index - i, LANCZOS_WINDOW_SIZE);
		}
...
Guacamole.Client = function (tunnel) {
	 ...
	 */
	this.sendMouseState = function sendMouseState(
		mouseState,
		applyDisplayScale,
	) {
		// Do not send requests if not connected
	 ...
	 *     {@link Guacamole.Display}.
	 */
	this.sendTouchState = function sendTouchState(
		touchState,
		applyDisplayScale,
	) {
		// Do not send requests if not connected
	...
	var layerPropertyHandlers = {
		...
		"multi-touch": function layerSupportsMultiTouch(layer, value) {
			...
			if (
				guac_client.onmultitouch &&
				layer instanceof Guacamole.Display.VisibleLayer
			)
	...
	var instructionHandlers = {
		...
		audio: function (parameters) {
			...
			if (!audioPlayer)
				audioPlayer = Guacamole.AudioPlayer.getInstance(
					stream,
					mimetype,
				);
		...
		transfer: function (parameters) {
			...
			/* Anything else that isn't a NO-OP */ else if (
				function_index !== 0x5
			)
				display.transfer(
					srcL,
	...
	var scheduleKeepAlive = function scheduleKeepAlive() {
		...
		var keepAliveDelay = Math.max(
			lastSentKeepAlive + KEEP_ALIVE_FREQUENCY - currentTime,
			0,
		);
	...
	this.disconnect = function () {
		...
		if (
			currentState != Guacamole.Client.State.DISCONNECTED &&
...
Guacamole.Display = function () {
	 ...
	 */
	var notifyFlushed = function notifyFlushed(
		localTimestamp,
		remoteTimestamp,
		logicalFrames,
	) {
		...
		for (var first = 0; first < statistics.length; first++) {
			if (
				current - statistics[first].timestamp <=
				guac_display.statisticWindow
			)
				break;
		}
		...
		// bound)
		var statDuration =
			(statistics[statistics.length - 1].timestamp -
		...
		// remote desktop)
		var remoteDuration =
			(statistics[statistics.length - 1].remoteTimestamp -
				statistics[0].remoteTimestamp) /
			1000;
		...
		var stats = new Guacamole.Display.Statistics({
			...
			desktopFps:
				remoteDuration && remoteFrames
					? remoteFrames / remoteDuration
					: null,
	...
	function Task(taskHandler, blocked) {
		...
		this.unblock = function () {
			if (task.blocked) {
				...
				if (frames.length) __flush_frames();
			}
		};
	...
	this.createLayer = function () {
		var layer = new Guacamole.Display.VisibleLayer(
			displayWidth,
			displayHeight,
		);
	...
	this.drawStream = function drawStream(layer, x, y, stream, mimetype) {
		...
		if (window.ImageDecoder && window.ReadableStream) {
			...
			imageDecoder
				.decode({ completeFramesOnly: true })
				.then(function bitmapLoaded(result) {
...
Guacamole.KeyEventInterpreter = function KeyEventInterpreter(startTimestamp) {
	 ...
	 */
	/**
	 * Returns data for get Unicode Key Definition.
	 */
	 ...
	 */
	/**
	 * Returns data for get Key Definition By Keysym.
	 */
	function getKeyDefinitionByKeysym(keysym) {
		// If it's a known type, return the existing definition
		if (keysym in KNOWN_KEYS) return KNOWN_KEYS[keysym];
	...
	this.handleKeyEvent = function handleKeyEvent(args) {
		...
		parsedEvents.push(
			new Guacamole.KeyEventInterpreter.KeyEvent({
...
Guacamole.Keyboard = function Keyboard(element) {
	...
	var KeydownEvent = function KeydownEvent(orig) {
		...
		if (
			!this.keysym &&
			key_identifier_sane(this.keyCode, this.keyIdentifier)
		)
		...
		if (
			quirks.altIsTypableOnly &&
			(this.keysym === 0xffe9 || this.keysym === 0xffea)
		)
			this.keysym = 0xfe03;
	...
	/**
	 * Handles keysym from key identifier.
	 */
	...
	this.listenTo = function listenTo(element) {
		...
		var handleInput = function handleInput(e) {
			...
			if (e.data && !e.isComposing) guac_keyboard.type(e.data);
		};
		...
		var handleCompositionEnd = function handleCompositionEnd(e) {
			...
			// Type all content written
			if (e.data) guac_keyboard.type(e.data);
		};
...
Guacamole.Mouse = function Mouse(element) {
	...
	element.addEventListener(
		...
		function (e) {
			...
			guac_mouse.move(
				Guacamole.Position.fromClientPosition(
					element,
					e.clientX,
					e.clientY,
				),
	...
	/**
	 * Handles ignore Pending Mouse Events.
	 */
...
Guacamole.Mouse.State = function State(template) {
	 ...
	 */
	var legacyConstructor = function legacyConstructor(
		x,
		y,
		left,
		middle,
...
Guacamole.Mouse.Event.Target = function MouseEventTarget() {
	...
	this.release = function release(button, events) {
		if (this.currentState[button]) {
			...
			this.dispatch(
				new Guacamole.Mouse.Event("mouseup", this.currentState, events),
			);
		}
	};
	...
	this.out = function out(events) {
		this.dispatch(
			new Guacamole.Mouse.Event("mouseout", this.currentState, events),
		);
	};
	...
	element.addEventListener(
		...
		function (e) {
			...
			if (touch_count === 1) {
				...
				var velocity =
					pixels_moved / (new Date().getTime() - last_touch_time);
...
Guacamole.Object = function guacamoleObject(client, index) {
	...
	this.createOutputStream = function createOutputStream(mimetype, name) {
		return client.createObjectOutputStream(
			guacObject.index,
			mimetype,
			name,
		);
	...
	var removeClass = function removeClass(element, classname) {
		...
		// Otherwise, manually filter out classes with given name
			...
			element.className = element.className.replace(
				/([^ ]+)[ ]*/g,
				/**
				 * Updates remove Matching Classes.
				 */
...
Guacamole.Parser = function Parser() {
	...
	this.receive = function receive(packet, isBuffer) {
		if (isBuffer) buffer = packet;
		else {
			// Truncate buffer as necessary
			if (
				startIndex > BUFFER_TRUNCATION_THRESHOLD &&
			...
			if (buffer.length) buffer += packet;
			else buffer = packet;
		}
		...
		while (elementEnd < buffer.length) {
			...
			if (elementEnd >= startIndex) {
				...
				// length.
				var codepoints = Guacamole.Parser.codePointCount(
					buffer,
					startIndex,
					elementEnd,
				);
				...
				// end and reevaluate.
					...
					elementCodepoints &&
					buffer.codePointAt(elementEnd - 1) >=
						MIN_CODEPOINT_REQUIRES_SURROGATE
				) {
					elementEnd++;
					continue;
				}
			...
			if (lengthEnd !== -1) {
				// Parse length
				elementCodepoints = parseInt(
					buffer.substring(elementEnd + 1, lengthEnd),
				);
...
Guacamole.Position = function Position(template) {
	 ...
	 */
	this.fromClientPosition = function fromClientPosition(
		element,
		clientX,
		clientY,
	) {
 ...
 *     client coordinates.
 */
Guacamole.Position.fromClientPosition = function fromClientPosition(
	element,
	clientX,
	clientY,
) {
 ...
 */
Guacamole.SessionRecording = function SessionRecording(
	source,
	refreshInterval,
) {
	// Default the refresh interval to 1 second if not specified otherwise
	if (refreshInterval === undefined) refreshInterval = 1000;
	 ...
	 *     the given blob.
	 */
	var parseBlob = function parseBlob(
		blob,
		instructionCallback,
		completionCallback,
	) {
	 ...
	 */
	/**
	 * Handles initialize Key Interpreter.
	 */
	...
	var loadInstruction = function loadInstruction(opcode, args) {
		...
		if (opcode === "sync") {
			...
			if (frames.length === 1) initializeKeyInterpreter(timestamp);
			...
			if (
				frames.length === 1 ||
				(frameEnd - frames[lastKeyframe].start >=
					KEYFRAME_CHAR_INTERVAL &&
					timestamp - frames[lastKeyframe].timestamp >=
		...
		} else if (opcode === "key") keyEventInterpreter.handleKeyEvent(args);
	};
		...
		parseBlob(
			...
			function replayCompleted() {
				...
				if (frame.keyframe && !frame.clientState) {
					playbackClient.exportState(
						function storeClientState(state) {
							frame.clientState = new Blob([
	...
	var seekToFrame = function seekToFrame(index, callback, nextRealTimestamp) {
		...
		var continueReplay = function continueReplay() {
			...
			if (recording.onseek && currentFrame > startIndex) {
				currentPosition = toRelativeTimestamp(
		...
		var continueAfterRequiredDelay = function continueAfterRequiredDelay() {
			var delay = nextRealTimestamp
				? Math.max(nextRealTimestamp - new Date().getTime(), 0)
				: 0;
			if (delay) {
		...
		if (currentFrame + 1 < frames.length) {
			...
			// The position needs to be incremented before the next frame
				...
				updateTimeout = window.setTimeout(function incrementPosition() {
					...
					if (recording.onseek) recording.onseek(currentPosition);
	...
	this.seek = function seek(position, callback) {
		...
		seekToFrame(closestFrame, function seekComplete() {
			...
			if (recording.onseek) recording.onseek(position);
	...
	element.addEventListener(
		...
		function touchstart(e) {
			...
			for (var i = 0; i < e.changedTouches.length; i++) {
				...
				touch.fromClientPosition(
					element,
					changedTouch.clientX,
					changedTouch.clientY,
				);
				guacTouch.dispatch(
					new Guacamole.Touch.Event("touchmove", e, touch),
				);
			}
		},
		false,
	...
	element.addEventListener(
		...
		function touchstart(e) {
			...
			for (var i = 0; i < e.changedTouches.length; i++) {
				...
				guacTouch.dispatch(
					new Guacamole.Touch.Event("touchend", e, touch),
				);
			}
		},
		false,
...
Guacamole.HTTPTunnel = function (tunnelURL, crossDomain, extraTunnelHeaders) {
	...
	/**
	 * Handles handle HTTPTunnel Error.
	 */
	function handleHTTPTunnelError(xmlhttprequest) {
		...
		if (code) {
			var message = xmlhttprequest.getResponseHeader(
	...
	function makeRequest() {
		...
		xmlhttprequest.open(
			"GET",
			TUNNEL_READ + tunnel.uuid + ":" + request_id++,
		);
	...
	this.connect = function (data) {
		...
		connect_xmlhttprequest.onreadystatechange = function () {
			...
			tunnelSessionToken =
				connect_xmlhttprequest.getResponseHeader(TUNNEL_TOKEN_HEADER);
...
Guacamole.WebSocketTunnel = function (tunnelURL) {
	...
	if (
		...
		// Otherwise, construct absolute from relative URL
			...
			tunnelURL =
				protocol + "//" + window.location.host + path + tunnelURL;
		}
	}
	...
	var resetTimers = function resetTimers() {
		...
		var currentTime = new Date().getTime();
		var pingDelay = Math.max(
			lastSentPing + PING_FREQUENCY - currentTime,
			0,
		);
...
Guacamole.ChainedTunnel = function (tunnelChain) {
	...
	function attach(tunnel) {
		...
		var failTunnel = function failTunnel(status) {
			...
			if (
				status &&
				status.code === Guacamole.Status.Code.UPSTREAM_TIMEOUT
			) {
				tunnels = [];
				return null;
			}
