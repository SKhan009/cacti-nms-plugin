...
$nms_template_section =
	isset($_GET["section"]) && is_string($_GET["section"]) && isset($nms_template_sections[$_GET["section"]])
		? $_GET["section"]
		: "input";
...
// Run the native controller as the top-level document and opt it into the NMS
// wrapper. This avoids Cacti frame-busting behavior while retaining native
// ACL, CSRF, AJAX, validation and save flows.
$destination = nms_cacti_url($route) . (strpos($route, "?") === false ? "?" : "&") . "nms_workspace=templates";
