...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset_request_var("nms_action")) {
	...
	if ($action === "acknowledge" && $id > 0) {
		try {
			...
			$incident_host = db_fetch_cell_prepared("SELECT host_id FROM plugin_nms_incidents WHERE id = ?", [$id]);
...
$allowed_states = ["all", "up", "fault", "acknowledged"];
...
$where = ["h.deleted = ''", "h.disabled = ''", $visible_hosts];
$params = [];
...
if ($search !== "") {
	$where[] =
		"(h.description LIKE ? OR h.hostname LIKE ? OR h.snmp_sysName LIKE ? OR s.name LIKE ? OR ht.name LIKE ? OR c.name LIKE ?)";
	$term = "%" . $search . "%";
...
$counts = db_fetch_row_prepared(
	"SELECT COUNT(*) AS total_count,
	SUM(h.status = " .
		HOST_UP .
		" AND h.last_updated BETWEEN ? AND ?) AS up_count,
	...
	[nms_parameter_fresh_after(), nms_now()],
);

$device_total = (int) $counts["total_count"];
$device_up = (int) $counts["up_count"];
$availability =
	$device_total > 0 ? round(($device_up / $device_total) * 100, 1) . "% currently up" : "No enabled devices";
...
foreach ($parameter_rows as $parameter) {
	...
	if (!isset($parameters_by_host[$host_id])) {
		$parameters_by_host[$host_id] = [];
	}
...
$last_sync = db_fetch_cell_prepared("SELECT updated_at FROM plugin_nms_meta WHERE meta_key = 'last_sync'", []);
...
<main class="nms-shell">
	<div class="nms-heading">
		...
		<div class="nms-health">
			...
			<div><strong><?php print nms_h(
   	$availability,
   ); ?></strong><small><?php print $device_up; ?> of <?php print $device_total; ?> devices with a current Up state</small></div>
		</div>
	</div>

	<div class="nms-summary-grid">
		<div class="nms-summary nms-summary-total"><span>Total devices</span><strong><?php print (int) $counts[
  	"total_count"
  ]; ?></strong><small>Enabled monitoring targets</small></div>
		<div class="nms-summary nms-summary-resolved"><span>Devices up</span><strong><?php print (int) $counts[
  	"up_count"
  ]; ?></strong><small>Responding normally</small></div>
		<div class="nms-summary nms-summary-critical"><span>Devices with faults</span><strong><?php print (int) $counts[
  	"fault_count"
  ]; ?></strong><small>Based on configured category rules</small></div>
	...
	<section class="nms-panel" id="incident-queue">
		<div class="nms-panel-head">
			<div><h2>Device status</h2><p><?php print count(
   	$devices,
   ); ?> matching devices · Last synchronized <?php print $last_sync
 	? nms_h(nms_time_ago($last_sync))
 	: "now"; ?></p></div>
		...
		<form class="nms-toolbar" method="get" action="nms.php">
			...
			<label class="nms-search"><span>⌕</span><input type="search" name="search" value="<?php print nms_h(
   	$search,
   ); ?>" placeholder="Search device name, address, or site"></label>
    	...
    	$status_label = $has_fault
    		? $device["incident_status"]
    		: (!$core_fresh
    			? "Stale"
    			: (!$is_up
    				? "Unavailable"
    				: ((int) $device["configured_rule_count"] > 0
    					? "No active faults"
    					: "Unconfigured")));
    	...
    	$detail = $has_fault
    		? trim((string) $device["incident_message"])
    		: ((int) $device["configured_rule_count"] === 0
    			? "No enabled equipment-category rules. Cacti collection state is shown separately."
    			: "No active rule incident; missing or stale measurements are not proof of recovery.");
    	if ($detail === "") {
    		$detail = "Cacti reports device state " . $status_name;
    	}
    	$device_parameters = isset($parameters_by_host[(int) $device["id"]])
    		? $parameters_by_host[(int) $device["id"]]
    		: [];
    	$fresh_parameters = [];
    	foreach ($device_parameters as $parameter) {
    		if ($is_up && nms_parameter_has_current_value($parameter)) {
    			$fresh_parameters[] = $parameter;
    		}
    	}
    	$device_inventory = isset($inventory_by_host[(int) $device["id"]])
    		? $inventory_by_host[(int) $device["id"]]
    		: [];
    	?>
				<tr>
					<td><div class="nms-incident"><i class="nms-severity <?php print nms_h(
     	$severity_class,
     ); ?>"></i><div><strong><?php print nms_h($device["description"]); ?></strong><small><?php print nms_h(
	($device["category_name"] ?: "Unmapped") . " · " . ($device["template_name"] ?: "No template"),
); ?></small><small><?php print nms_h($detail); ?></small></div></div></td>
					<td><strong><?php print nms_h($device["hostname"]); ?></strong><?php if (
	$device["site_name"]
) { ?><small><?php print nms_h($device["site_name"]); ?></small><?php } ?></td>
					<td><span class="nms-state <?php print nms_h($status_class); ?>"><?php print nms_h(
	$status_label,
); ?></span><small>Cacti device: <?php print nms_h($status_name); ?></small><?php
if ($has_fault && (int) $device["active_fault_count"] > 1) { ?><small><?php print (int) $device[
	"active_fault_count"
]; ?> active faults</small><?php }
if ($device["acknowledged_by_name"]) { ?><small>by <?php print nms_h($device["acknowledged_by_name"]); ?></small><?php }
?></td>
					<td><strong><?php print nms_h(
     	number_format((float) $device["availability"], 1),
     ); ?>%</strong><small><?php print (int) $device["failed_polls"]; ?> failed</small></td>
					<td><strong><?php print count($fresh_parameters); ?> current readings</strong><?php
 if (count($fresh_parameters) && $is_up) {
 	foreach (array_slice($fresh_parameters, 0, 2) as $parameter) { ?><small title="<?php print nms_h(
	$parameter["display_name"],
); ?>"><?php print nms_h($parameter["parameter_name"]); ?>: <?php print nms_h(
	$parameter["raw_value"],
); ?></small><?php }
 } elseif (!$is_up) { ?><small>No current value — Cacti reports <?php print nms_h(
	$status_name,
); ?></small><?php } elseif (
 	count($device_parameters)
 ) { ?><small>No current value — last Cacti readings are stale</small><?php } else { ?><small>Waiting for the next Cacti poll</small><?php }
 if (isset($device_inventory["serial_number"])) {
 	$serial = $device_inventory["serial_number"];
 	$serial["status"] = nms_serial_observation_state(
 		$serial["status"],
 		$serial["last_success"],
 		$device["status"],
 		$device["disabled"],
 	...
 	if ($serial["status"] === "failed") { ?><small>Serial: live SNMP read failed</small><?php } elseif (
 		!in_array($serial["status"], ["ok", "changed"], true)
 	) { ?><small>Serial: <?php print nms_h(
	$serial["status"],
); ?> — no current value</small><?php } else { ?><small>Serial: <?php
print nms_h($serial["observed_value"]);
print $serial["status"] === "changed" ? " (changed)" : "";
?></small><?php }
 }
 ?></td>
					<td class="nms-nowrap" title="<?php print nms_h($device["last_updated"]); ?>"><?php print nms_h(
	nms_time_ago($device["last_updated"]),
); ?></td>
					<td>
					...
					<?php } elseif (!$has_fault) { ?><span class="nms-muted"><?php print nms_h(
	$status_label,
); ?></span><?php } else { ?><span class="nms-muted">Acknowledged</span><?php } ?>
					</td>
				</tr>
				<?php
    } ?>
