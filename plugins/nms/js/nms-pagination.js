...
/** Initialize client-side pagination for NMS tables and repeated lists. */ (function () {
	...
	function directChildren(container, selector) {
		return Array.prototype.filter.call(
			container.children,
			/** Keep matching rows that do not contain an empty-state message. */ function (
				child,
			) {
				return (
					child.matches(selector) &&
					!child.querySelector(".nms-empty")
				);
			},
		);
	}
	...
	function pageTokens(current, total) {
		if (total <= 7)
			return Array.from(
				{ length: total },
				/** Convert a zero-based array index into a displayed page number. */ function (
					_,
					index,
				) {
					return index + 1;
	...
	function initialize() {
		...
		Array.prototype.forEach.call(
			document.querySelectorAll(".nms-association-table"),
			/** Paginate association entries without counting their heading row. */ function (
				container,
				index,
			) {
				new Paginator(
					container,
					".nms-association-row:not(.heading)",
					"Association list " + (index + 1),
				);
			},
		);
		Array.prototype.forEach.call(
			document.querySelectorAll(".nms-existing-rules"),
			/** Paginate the compact fault-rule entries in this container. */ function (
				container,
			) {
		...
		Array.prototype.forEach.call(
			document.querySelectorAll(".nms-popup-categories"),
			/** Paginate the device-category entries in this container. */ function (
				container,
			) {
				new Paginator(container, "div", "Device segments");
			},
		);
