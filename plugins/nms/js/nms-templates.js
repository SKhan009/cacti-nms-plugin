...
(function () {
	"use strict";
	/** Mount the NMS navigation around the untouched native Cacti template editor. */
	function init() {
		var configNode = document.getElementById("nmsNativeConfig");
		var main = document.getElementById("main");
		if (!configNode || !main || document.getElementById("nmsSidebar"))
			return;
		...
		description.textContent =
			"Templates: data input method → data source → graph → device. Use data queries for indexed tables. All fields and saves below are provided by Cacti core.";
		...
		/** Highlight the template submenu that corresponds to the current native editor URL. */
		function syncSection() {
			var section =
				config.pages[location.pathname.slice(config.base.length)];
			document
				.querySelectorAll(".nms-template-subnav a")
		...
		document
			.querySelectorAll(".nms-template-menu > summary")
		...
		/** Preserve opt-in workspace context only for same-installation native template routes. */
		function workspaceURL(value) {
			try {
				...
				if (
					...
					config.pages[page] &&
					url.searchParams.get("nms_workspace") !== "off"
				)
					url.searchParams.set("nms_workspace", "templates");
				return url.href;
			} catch (_) {
				return value;
			}
		...
		["pushState", "replaceState"].forEach(function (method) {
			...
			history[method] = function (state, title, url) {
				var result = nativeMethod.call(
					history,
					state,
					title,
					url ? workspaceURL(url) : url,
				);
				syncSection();
				return result;
			};
		...
		/** Reapply non-destructive presentation enhancements after native AJAX updates. */
		function decorate() {
			var listTable = main.querySelector(
				"table.cactiTable th.tableSubHeaderCheckbox",
			);
			...
			main.querySelectorAll(".actionsDropdown").forEach(
				function (actions) {
					...
					if (
						!form ||
						!actions.querySelector('select[name="drp_action"]')
					)
						return;
					...
					if (actions.nextElementSibling !== anchor)
						form.insertBefore(actions, anchor);
				},
			);
		...
		if (window.jQuery)
			window.jQuery.ajaxPrefilter(function (options) {
				options.url = workspaceURL(options.url);
			});
	}
