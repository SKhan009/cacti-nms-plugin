...
<div class="nms-canvas-toolbar"><strong>Network topology</strong><span id="nms-topology-count"></span><div class="nms-canvas-controls"><?php if (is_realm_allowed(3)) { ?><button type="button" id="nms-edit-mode" aria-pressed="false">Edit mode</button><button type="button" id="nms-undo" aria-label="Undo device move" title="Undo device move" disabled>↶</button><button type="button" id="nms-redo" aria-label="Redo device move" title="Redo device move" disabled>↷</button><a class="nms-catalog-button nms-edit-connections" href="topology.php?tab=connections">Connections</a><?php } ?><button type="button" id="nms-zoom-out" aria-label="Zoom out">−</button><button type="button" id="nms-zoom-in" aria-label="Zoom in">+</button><button type="button" id="nms-fit">Fit</button><button type="button" id="nms-map-refresh">Refresh</button><button type="button" id="nms-fullscreen" aria-pressed="false">Full screen</button></div></div>
